(function () {
  ...
  const renderers={surface:surfaceView,overview,ecosystem,lineage,cohorts,centrality,quality,pulse};
  ...
  async function init(){try{
    const [dashboardResponse,v2Response,surfaceResponse]=await Promise.all([
      fetch('research/dashboard_data.json'),
      fetch('research/data/research_v2.json'),
      fetch('research/surface_analytics_v1.json')
    ]);
    if(!dashboardResponse.ok||!v2Response.ok||!surfaceResponse.ok)throw new Error('load');
    [data,researchV2,surface]=await Promise.all([
      dashboardResponse.json(),
      v2Response.json(),
      surfaceResponse.json()
    ]);
