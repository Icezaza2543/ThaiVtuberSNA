...
const output=process.env.QA_OUTPUT || '.tmp/research-ui';await mkdir(output,{recursive:true});
...
for(const width of [1440,1280,1024,390]) {
 ...
 await page.goto(process.env.QA_URL || 'http://127.0.0.1:5500/?view=research');await page.waitForSelector('#kpiStrip .kpi-value');
...
assert.deepEqual(errors,[]);await writeFile(output+'/results.json',JSON.stringify({report,errors},null,2));console.log('Research UI checks passed');
